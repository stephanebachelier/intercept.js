var Intercept = { // jshint unused:false
  ...
  start: function (options) {
    ...
    if (options.links) {
      this._getRootElement().addEventListener('click', _.bind(this._interceptLinks, this));
    }
    if (options.forms) {
      this._getRootElement().addEventListener('submit', _.bind(this._interceptForms, this));
    }
  },

  stop: function () {
    this._getRootElement().removeEventListener('click', this._interceptLinks);
